...
import { makeEvidence } from '../../adapters/evidence.mjs';
...
test('V1 golden booking/payment SaaS reaches OPA PASS with current external evidence', async () => {
  ...
  const operations = makeEvidence({ kind: 'operations', claim: 'rollback-and-recovery-verified', source: 'golden-runner', runner: 'golden-runner', scope: { commit }, status: 'PASS' });

  const verification = Object.fromEntries(designCriteria.map((criterion) => [criterion, true]));
  const design = verifyDesign({ verification });
